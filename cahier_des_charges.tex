...
\geometry{left=2.5cm, right=1cm, top=2.5cm, bottom=2.5cm}
...
\paragraph{}Le moteur combine des règles métiers (annuités, DTI/LTV, pondérations de stabilité pro et apport) et un micro-modèle ML entraîné à la volée sur un dataset synthétique. L'objectif est de produire un avis automatique cohérent dès la simulation, tout en restant explicable et ajustable par l'utilisateur (suggestion si le DTI est trop élevé).
\begin{itemize}[leftmargin=1.3cm]
    ...
    \item \textbf{Score heuristique} : pondération des critères (DTI, LTV, stabilité pro, bonus propriétaire, historique Banquise).
    \item \textbf{Suggestion DTI 35\%} : si la mensualité dépasse le DTI cible (35\%), calcul d'une mensualité conseillée et d'une durée ajustée (plafond 40 ans) proposée à l'utilisateur.
\end{itemize}

\section{Modèle ML simplifié – runtime (ancienne version}
En complément des règles heuristiques, un petit modèle de régression logistique est entraîné à l'exécution sur un jeu de données synthétique :
\begin{itemize}[leftmargin=1.3cm]
    \item Génération de 600 profils aléatoires (revenus 1-12 k€, DTI 10-70\%, LTV 50-110\%, apport 0-60\%).
    \item Étiquette "bon profil" si au moins 3 critères sur 4 sont satisfaits (revenus $>$ 4 k€, DTI $<$ 40\%, LTV $<$ 90\%, apport $>$ 20\%).
    \item Normalisation des variables, ajout d'un biais, descente de gradient (300 itérations, lr = 0{,}05) pour obtenir les poids \texttt{\_ML\_WEIGHTS}.
    \item Au moment de la simulation, le modèle est chargé si absent, produit un score 0-100 (\texttt{\_ml\_score}) à partir des features normalisées (revenus, DTI, LTV, ratio apport) ; ce score est moyenné avec le score heuristique pour l'\textbf{avis automatique}.
\end{itemize}

\section{Seuils et décisions}
\begin{itemize}[leftmargin=1.3cm]
    \item DTI seuil : 42\% (revenus $<$ 6000€) ou 47\% (revenus $\geq$ 6000€).
    \item LTV seuil : 95\%, porté à 97\% si montant $\geq$ 250k€ et apport $\geq$ 10\%.
    \item Score minimal : 55/100 pour une acceptation automatique (avis IA).
...
\begin{enumerate}[leftmargin=1.3cm]
    ...
    \item \textbf{Entraînement hors-ligne} (\texttt{ml/train\_credit\_model.py}) : construction d'un \textit{pipeline} scikit-learn (prétraitement + modèle). Trois modèles testés (régression logistique, arbre, random forest). Sélection du meilleur sur ROC AUC/F1, puis sauvegarde du pipeline complet (prépro + modèle) avec \texttt{joblib} dans \texttt{scoring/model\_credit.pkl}.
...
\begin{itemize}[leftmargin=1.5cm]
    ...
    \item Calcul automatique du score de solvabilité : heuristique embarquée + modèle ML entraîné hors-ligne (pipeline scikit-learn sérialisé)
    ...
    \item Versement automatique sur le premier compte actif si l'admin accepte
    \item Prélèvements mensuels automatiques sur les crédits acceptés (rattrapage des mensualités manquantes)
...
Le calcul du score de crédit combine une heuristique et un micro-modèle logistique entraîné à la volée :
\begin{itemize}[leftmargin=1.5cm]
    \item \textbf{DTI} : mensualité + charges / revenus (pénalité au-delà de 30\%, seuil dynamique 42\% ou 47\% selon revenus)
    \item \textbf{LTV} : pénalité au-delà de 85\% (seuil jusqu'à 97\% si montant $\geq$ 250k€ et apport $\geq$ 10\%)
    \item \textbf{Revenus et apport} : bonus si apport $\geq$ 20\% ; malus si revenus $<$ 2000€
    \item \textbf{Profil} : bonus CDI, bonus propriétaire, bonus santé BON
    \item \textbf{Modèle ML} : régression logistique sur dataset synthétique (revenus, DTI, LTV, ratio apport), moyenné avec le score heuristique
    \item \textbf{Décision IA} : score final $\geq$ 55 $\Rightarrow$ avis auto accepté ; sinon refus (statut métier reste EN\_ATTENTE jusqu’à validation admin)
    \item \textbf{Suggestion} : si DTI dépasse 35\%, proposition d'une mensualité cible et d'une durée ajustée (plafond 40 ans)
\end{itemize}

\paragraph{Formule heuristique (profil “bon”)} Le score heuristique est initialisé à 100 et pénalisé/bonifié ainsi :
{\small
\begin{multline*}
\text{score\_h} = \max\Big(0,\ \min\big(100,\ 100 - \underbrace{\max(0,DTI-30)}_{\text{malus DTI}} - \underbrace{0{,}25\times \max(0,LTV-85)}_{\text{malus LTV}} \\
- \underbrace{\mathbf{1}_{revenus<2000}\times 8}_{\text{revenus faibles}} + \underbrace{\mathbf{1}_{apport\ge 20\%}\times 10}_{\text{bonus apport}} + \underbrace{\mathbf{1}_{CDI}\times 10}_{\text{bonus CDI}} + \underbrace{\mathbf{1}_{proprio}\times 10}_{\text{bonus logement}} + \underbrace{\mathbf{1}_{santé=BON}\times 2}_{\text{bonus santé}} \big)\Big)
\end{multline*}
}
Le score final est la moyenne \(\frac{\text{score\_heur} + \text{score\_ml}}{2}\) (si \texttt{numpy} indisponible : score heuristique seul) ; un crédit est considéré “bon” par l’IA si le score final $\geq 55$.

\paragraph{Extrait de code (seuils et avis IA)} 
\begin{verbatim}
final_score = int((score + (ml_score if ml_score else score)) / 2)
ia_decision = 'ACCEPTEE' if final_score >= 55 else 'REFUSEE'
dti_limit = 42 if revenus_mensuels < 6000 else 47
ltv_limit = 97 if montant >= 250000 and apport >= 0.10*montant else 95
\end{verbatim}
Cet extrait montre le calcul du score final, l'avis IA et les seuils DTI/LTV dynamiques.

\subsection{Cycle de vie du crédit}
\begin{enumerate}
    \item Client remplit le formulaire de simulation
    \item Système calcule le score et génère un avis IA (score final $\geq$ 55 = avis accepté)
...
\hline
\texttt{ml/train\_credit\_model.py} & Script d'entraînement offline (pandas/scikit-learn) pour générer \texttt{model\_credit.pkl} \\
\hline
...
|-- ml/                    # Scripts d'entraînement (train_credit_model.py)
|-- scoring/model_credit.pkl # Pipeline ML sérialisé (ou générer via ml/train_credit_model.py)
...
\subsection{Inscription en deux étapes (code email)}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{inscription_code_email.png}
    \caption{Inscription : envoi du code et saisie pour activation}
    \label{fig:inscription-2etapes}
\end{figure}

\subsection{Dashboard avec projections crédits}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{dashboard_projections_credits.png}
    \caption{Dashboard client : dépenses 6/12 mois et projections de mensualités}
    \label{fig:dashboard-projections}
\end{figure}

\subsection{Blocage automatique des cartes (découvert)}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{blocage_cartes_decouvert.png}
    \caption{Alerte 80\% et blocage/déblocage automatique des cartes}
    \label{fig:blocage-cartes-decouvert}
\end{figure}

\subsection{Virement et bénéficiaires}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{virement_beneficiaire_iban.png}
    \caption{Formulaire de virement : bénéficiaire enregistré ou IBAN saisi}
    \label{fig:virement-beneficiaire}
\end{figure}

\subsection{Découvert temporaire}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{demande_decouvert_temporaire.png}
    \caption{Demande de découvert temporaire (client) et décision admin}
    \label{fig:decouvert-temporaire}
\end{figure}

\subsection{Simulation crédit et suggestion DTI 35\%}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{simulation_credit_suggestion.png}
    \caption{Simulation crédit : score, DTI/LTV, suggestion de mensualité/durée}
    \label{fig:simulation-credit-suggestion}
\end{figure}

\subsection{Console admin crédits et exports}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{console_admin_credits_exports.png}
    \caption{Console admin : acceptation/refus crédits, exports CSV, blocage massif}
    \label{fig:console-credits-exports}
\end{figure}

\subsection{Chat support (client et admin)}
\begin{figure}[H]
    \centering
    \includegraphics[width=\textwidth]{chat_support_pj_edit.png}
    \caption{Chat support : pièce jointe image et message édité (client/admin)}
    \label{fig:chat-support}
\end{figure}
...
\begin{titlepage}
    ...
    \begin{tabular}{@{}p{3cm}p{10cm}@{}}
        ...
        \textbf{Livrables} & Cahier des charges, architecture, guide utilisateur/admin, pipeline ML sérialisé, templates et scripts d'entraînement. \\[0.25cm]
