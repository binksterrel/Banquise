...
class Command(BaseCommand):
    help = "Entraîne le modèle de scoring crédit à partir du dataset réel."
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(f"Chargement du dataset : {data_path}")

        if not data_path.exists():
            self.stderr.write(self.style.ERROR(f"Dataset non trouvé : {data_path}"))
            return

        # --- 1. CHARGEMENT ---
        df = pd.read_csv(data_path)
        self.stdout.write(f"Dataset chargé : {len(df)} lignes")
        ...
        if "Loan_Status" not in df.columns:
            self.stderr.write(self.style.ERROR("Colonne 'Loan_Status' manquante."))
            return
        ...
        def calculate_pmt_vectorized(principal, months, rate=0.035):
            ...
            # If rate is 0, pmt = P/N. Here rate is fixed 3.5%
            # N must be > 0
        ...
        self.stdout.write(f"Taux positif (acceptés) : {y.mean() * 100:.1f}%")
        ...
        self.stdout.write(self.style.SUCCESS(f"Modèle sauvegardé : {output_path}"))
